...
void eval(char *cmdline) {
    int groupID = 0;
    char *args[MAXARGS];
    int children = parseline(cmdline, args);
    sigset_t signalMask;
    ...
    // If there are aguments continue to evaluate
    } else {
        ...
        if (!builtin) {
            ...
            sigprocmask(SIG_BLOCK, &signalMask, &originalMask);
            
            int processID = fork();

            if (!processID) {

                setpgid(groupID, groupID);
            ...
            if (!children) {
                addjob(jobs, processID, groupID, FG, cmdline);
                sigprocmask(SIG_SETMASK, &originalMask, NULL);
                waitfg(processID);
                
            } else {
                addjob(jobs, processID, groupID, BG, cmdline);
                sigprocmask(SIG_SETMASK, &originalMask, NULL);
...
void sigchld_handler(int sig) {
    ...
    while((foregroundPID = waitpid(-1, &completion, WNOHANG | WUNTRACED)) > 0) {
        if (WIFEXITED(completion)) {
            deletejob(jobs, foregroundPID);

        } else if (WIFSTOPPED(completion)) {
            printf("Job [%d] (%d) stopped by signal %d\n", pid2jid(foregroundPID), foregroundPID, WSTOPSIG(completion));
            getjobpid(jobs,foregroundPID)->state = ST;

        } else if (WIFSIGNALED(completion)) {
            printf("Job [%d] (%d) terminated by signal %d\n", pid2jid(foregroundPID), foregroundPID, WTERMSIG(completion));
            deletejob(jobs, foregroundPID);
        }
    }
