...
void eval(char *cmdline) {
    ...
    // If there are arguments continue to evaluate
    } else {
        ...
        if (!builtin) {
            int in[MAXJOBS];
            int out[MAXJOBS];
            int commands[MAXJOBS];
            int count = parseargs(args, commands, in, out);
            int parent = 0;
            ...
            for (int i = 0; i < count; i++) {
                ...
                if (processID == 0) {
                    ...
                    if (i != (count - 1)) {
                        close(descriptor[0]);
                        dup2(descriptor[1], fileno(stdout));
                        
                        if (i > 0) {
                            dup2(parent, fileno(stdin));
                        }
                    ...
                    if (in[i] != -1) {
                        FILE* inFile = fopen(args[in[i]], "r");
                        int descriptionIn = fileno(inFile);
                        dup2(descriptionIn, 0);
                    }
                    
                    if (out[i] != -1) {
                        FILE* outFile = fopen(args[out[i]], "w");
                        int descriptionOut = fileno(outFile);
                        dup2(descriptionOut, 1);
                    }
                    
                    if (execve(args[commands[i]], &args[commands[i]], NULL) < 0) {
                        fprintf(stdout, "%s: Error. Command not found.\n", args[commands[i]]);
                        exit(0);
                    }
